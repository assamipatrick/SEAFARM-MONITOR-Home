...
import type { SeaweedType, ExportDocument, Site, CreditType, FarmerCredit, Repayment, Farmer, CultivationCycle, Module, Employee, Role, MonthlyPayment, IncidentSeverity } from '../../types';
...
// Main Component
export const GlobalFarmReport: FC<{ month: number; year: number }> = ({ month, year }) => {
    ...
    const { pivotData, sortedSeverities, displaySites, displaySeaweedTypes, onSiteStockReportData, pressedWarehouseReportData, monthlyProductionData } = useMemo(() => {
            ...
            if (incDate >= periodInfo.startDate && incDate <= periodInfo.endDate) {
                 ...
                 affectedPairs.forEach(pair => {
                     ...
                     if (structure[stId] && structure[stId][sId]) {
                        structure[stId][sId].incidents[inc.severity] = (structure[stId][sId].incidents[inc.severity] || 0) + 1;
                        structure[stId]['TOTAL'].incidents[inc.severity] = (structure[stId]['TOTAL'].incidents[inc.severity] || 0) + 1;
                     }
                 });
            }
         });
        ...
        return { pivotData: structure, sortedSeverities, displaySites, displaySeaweedTypes, onSiteStockReportData, pressedWarehouseReportData, monthlyProductionData: monthlyProd };
...
const CombinedOverviewPage: FC<any> = ({ period, pivotData, displaySites, displaySeaweedTypes, sortedSeverities, nextMonths, t, page, totalPages }) => {
    const { settings } = useSettings();
                ...
                <div className="w-full overflow-hidden">
                    <table className="w-full border-collapse mb-2 table-fixed text-[8px]">
                        ...
                        <tbody>
                            ...
                            <tr><Cell colSpan={colCount} className="bg-gray-200 text-black font-bold py-1.5 text-center text-[9px] border-t-2 border-gray-600 tracking-widest">{t('monitoringReport').toUpperCase()}</Cell></tr>
                            
                            {/* Incident Severity Breakdown */}
                            <tr>
                                <Cell colSpan={colCount} header align="left" className="pl-2 bg-gray-200 uppercase text-[8px] tracking-wider font-bold">{t('incidentsLabel')} - {t('severity')}</Cell>
                            </tr>
                            {sortedSeverities.map((sev: IncidentSeverity) => (
                                <tr key={sev.id}>
                                    <Cell align="left" className="pl-4 font-medium border-r border-gray-300">
                                        <span className={
                                            sev.id === 'CRITICAL' ? 'text-red-700 font-bold' :
                                            sev.id === 'HIGH' ? 'text-orange-600 font-bold' :
                                            sev.id === 'MEDIUM' ? 'text-yellow-600' : 'text-blue-600'
                                        }>{sev.name}</span>
                                    </Cell>
                                    {displaySeaweedTypes.map((st: any) => (
                                        <React.Fragment key={st.id}>
                                            {displaySites.map((s: any) => {
                                                const count = getD(st.id, s.id)?.incidents[sev.id];
                                                return <Cell key={s.id} className={count ? "font-bold bg-gray-50" : "text-gray-300"}>{count || '-'}</Cell>
                                            })}
                                            <Cell bold className="bg-gray-50 border-l border-gray-300">{getD(st.id, 'TOTAL')?.incidents[sev.id] || '-'}</Cell>
                                        </React.Fragment>
                                    ))}
                                </tr>
                            ))}
...
const Page2: FC<any> = ({ period, employees, sites, roles, page, totalPages, monthlyPayments, month, year }) => {
    ...
    // NEW: Calculate end of report period to filter out employees hired after this date
    const reportEndDate = useMemo(() => new Date(year, month + 1, 0, 23, 59, 59), [year, month]);
    ...
    const getStats = useCallback((roleName: string, category: 'PERMANENT' | 'CASUAL', siteId?: string) => {
        const filtered = employees.filter((e: Employee) => {
             // FIX: Filter by Hire Date to ensure employee was active during the report period
            if (e.hireDate) {
                 const hDate = new Date(e.hireDate);
                 if (hDate > reportEndDate) return false;
            }
        ...
        const amount = filtered.reduce((sum: number, e: Employee) => sum + getAmount(e), 0);
        
        return { count, amount };
    }, [employees, paymentMap, reportEndDate]);
